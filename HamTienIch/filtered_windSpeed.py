...
try:
    min_pressure = float(input("Nhập giá trị tối thiểu của Wind Speed (mph): "))
    max_pressure = float(input("Nhập giá trị tối đa của Wind Speed (mph): "))
except ValueError:
    print("Vui lòng nhập số hợp lệ.")
    exit()

# Lọc dữ liệu theo khoảng giá trị
filtered_data = data[(data['Wind Speed'] >= min_pressure) & 
                     (data['Wind Speed'] <= max_pressure)]
